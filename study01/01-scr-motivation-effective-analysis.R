...
lapply(list_dvs, function(dv) {
  ...
  write_nonparam_statistics_analysis_in_latex(
    ...
    , in_title = paste0(" for the scores of ", dv
                        , " in the first study for students with effective participation")
  )
})
...
# Validate Assumptions - fail Pressure/Tension
lapply(list_dvs, function(dv) {
  info <- list_info[[dv]]$info
...
df_assumptions <- get_dataframe_assumptions(
  dat = winsor_mod$wdat, from = 44
  , wid="UserID", dv="Pressure/Tension", iv="Type", between = c("Type","CLRole")
  , path = "data/scr-effective-pressure-tension", generate = T)
(normality_df <- df_assumptions[df_assumptions$normality.fail != T,])
(presure_non_index <- winsor_mod$wdat$UserID[!winsor_mod$wdat$UserID %in% winsor_mod$wdat$UserID[as.numeric(strsplit(paste0(normality_df$name[[1]]),"[+]")[[1]])]])

# removing non-normal data Pressure/Tension c(10179, 10183, 10186, 10213)
skip_stop <- c()
extra_rmids <- list(
  "Pressure/Tension" = presure_non_index
)
...
write_winsorized_in_latex(
  ...
  , in_title = paste("for intrinsic motivation score in the first study for students with effective participation")
)
...
lapply(list_dvs, function(dv) {
  ...
  write_param_statistics_analysis_in_latex(
    ...
    , in_title = paste0(" for ", dv, " score in the first study for students with effective participation")
  )
})
...
write_param_and_nonparam_statistics_analysis_in_latex(
  ...
  , in_title = "in the first study for students with effective participation"
  , min_size_tests = T)
