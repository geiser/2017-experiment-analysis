
#install.packages('dplyr')
#install.packages('RMySQL')
#install.packages('reshape')
#install.packages("devtools")
#devtools::install_github("mattsigal/careless")

library(readr)
library(dplyr)
library(RMySQL)
library(reshape)
library(careless)

## gather data from MySQL
con <- dbConnect(RMySQL::MySQL(), user = "root" , password = "qaz123456"
                 , dbname = "caede741_geiser_moodle", host = "localhost")

dbDisconnect(con)

## gather data from csv files
respIMI <- read_csv('2017-RespMotivation01.csv')

careless(select(respIMI, starts_with("Item")), append=FALSE) 
respIMI <- respIMI[-c(62,63,64,65),] # outliers

activities <- read_csv('2017-CLActivity01.csv')

## combine data for analysis
dat <- merge(
select(participants
, starts_with("UserID")
, starts_with("Type")
, starts_with("CLRole")
),
select(activities
, starts_with("UserID")
, starts_with("ParticipationLevel")
), by = "UserID")
dat <- merge(dat, select(respIMI, starts_with("UserID"), starts_with("Item")), by = "UserID")

###############################################################
## Reliability Analysis via Exploratory Factorial Analysis   ##
###############################################################

library(psych)

## validating sampling adequacy
KMO(cor(select(dat, starts_with("Item"))))

## factorial analysis with nfactor=4
fa(select(dat, starts_with("Item")), nfactors = 4)
fa(select(dat, starts_with("Item")
, -starts_with("Item04")
, -starts_with("Item07")
, -starts_with("Item13")
, -starts_with("Item19")), nfactors = 4)

fa(select(dat, starts_with("Item")
, -starts_with("Item03")
, -starts_with("Item05")
, -starts_with("Item04")
, -starts_with("Item07")
, -starts_with("Item13")
, -starts_with("Item19")), nfactors = 4)

fa(select(dat, starts_with("Item")
, -starts_with("Item20")
, -starts_with("Item23")
, -starts_with("Item03")
, -starts_with("Item05")
, -starts_with("Item04")
, -starts_with("Item07")
, -starts_with("Item13")
, -starts_with("Item19")), nfactors = 4)

fa(select(dat, starts_with("Item")
, -starts_with("Item23")
, -starts_with("Item24")
, -starts_with("Item05")
, -starts_with("Item07")
, -starts_with("Item13")), nfactors = 4)

fa(select(dat, starts_with("Item")
, -starts_with("Item19")
, -starts_with("Item23")
, -starts_with("Item24")
, -starts_with("Item05")
, -starts_with("Item07")
, -starts_with("Item13")), nfactors = 4)

## factorial analysis with nfactor=3
fa(select(dat, starts_with("Item")), nfactors = 3)
fa(select(dat, starts_with("Item")
, -starts_with("Item04")
, -starts_with("Item07")
, -starts_with("Item13")), nfactors = 3)

fa(select(dat, starts_with("Item")
, -starts_with("Item03")
, -starts_with("Item10")
, -starts_with("Item04")
, -starts_with("Item07")
, -starts_with("Item13")), nfactors = 3)

fa(select(dat, starts_with("Item")
, -starts_with("Item19")
, -starts_with("Item03")
, -starts_with("Item10")
, -starts_with("Item04")
, -starts_with("Item07")
, -starts_with("Item13")), nfactors = 3)

##
rdat <- select(dat, starts_with("UserID"), starts_with("Type"), starts_with("CLRole")
, starts_with("ParticipationLevel"), starts_with("Item")
, -starts_with("Item19")
, -starts_with("Item03")
, -starts_with("Item10")
, -starts_with("Item04")
, -starts_with("Item07")
, -starts_with("Item13"))

## reliability analysis - Intrinsic Motivation
inv_keys = c("Item17", "Item15", "Item08", "Item02", "Item06", "Item20"
, "Item16",  "Item14",  "Item18")
alpha <- alpha(select(rdat, starts_with("Item")), keys = inv_keys)
cat("Intrinsic Motivation", "\n"); alpha$total
alpha <- alpha(select(rdat[rdat$Type=="ont-gamified",], starts_with("Item")), keys = inv_keys)
cat("Intrinsic Motivation", ">>", "ont-gamified", "\n"); alpha$total
alpha <- alpha(select(rdat[rdat$Type=="non-gamified",], starts_with("Item")), keys = inv_keys)
cat("Intrinsic Motivation", ">>", "non-gamified", "\n"); alpha$total

## reliability analysis - Interest/Enjoyment
inv_keys = c()
alpha <- alpha(select(rdat
, starts_with("Item12"), starts_with("Item24")
, starts_with("Item01"), starts_with("Item09")
, starts_with("Item22"), starts_with("Item21")), keys = inv_keys)
cat("Interest/Enjoyment", "\n"); alpha$total
alpha <- alpha(select(rdat[rdat$Type=="ont-gamified",]
, starts_with("Item12"), starts_with("Item24")
, starts_with("Item01"), starts_with("Item09")
, starts_with("Item22"), starts_with("Item21")), keys = inv_keys)
cat("Interest/Enjoyment", ">>", "ont-gamified", "\n"); alpha$total
alpha <- alpha(select(rdat[rdat$Type=="non-gamified",]
, starts_with("Item12"), starts_with("Item24")
, starts_with("Item01"), starts_with("Item09")
, starts_with("Item22"), starts_with("Item21")), keys = inv_keys)
cat("Interest/Enjoyment", ">>", "non-gamified", "\n"); alpha$total

## reliability analysis - Perceived Choice
inv_keys = c("Item17", "Item15", "Item08", "Item02", "Item06", "Item20")
alpha <- alpha(select(rdat
, starts_with("Item17"), starts_with("Item15")
, starts_with("Item08"), starts_with("Item02")
, starts_with("Item06"), starts_with("Item20")
, starts_with("Item23"), starts_with("Item05")), keys = inv_keys)
cat("Perceived Choice", "\n"); alpha$total
alpha <- alpha(select(rdat[rdat$Type=="ont-gamified",]
, starts_with("Item17"), starts_with("Item15")
, starts_with("Item08"), starts_with("Item02")
, starts_with("Item06"), starts_with("Item20")
, starts_with("Item23"), starts_with("Item05")), keys = inv_keys)
cat("Perceived Choice", ">>", "ont-gamified", "\n"); alpha$total
alpha <- alpha(select(rdat[rdat$Type=="non-gamified",]
, starts_with("Item17"), starts_with("Item15")
, starts_with("Item08"), starts_with("Item02")
, starts_with("Item06"), starts_with("Item20")
, starts_with("Item23"), starts_with("Item05")), keys = inv_keys)
cat("Perceived Choice", ">>", "non-gamified", "\n"); alpha$total

## reliability analysis - Pressure/Tension
inv_keys = c("Item11")
alpha <- alpha(select(rdat
, starts_with("Item16"), starts_with("Item14")
, starts_with("Item18"), starts_with("Item11")), keys = inv_keys)
cat("Pressure/Tension", "\n"); alpha$total
alpha <- alpha(select(rdat[rdat$Type=="ont-gamified",]
, starts_with("Item16"), starts_with("Item14")
, starts_with("Item18"), starts_with("Item11")), keys = inv_keys)
cat("Pressure/Tension", ">>", "ont-gamified", "\n"); alpha$total
alpha <- alpha(select(rdat[rdat$Type=="non-gamified",]
, starts_with("Item16"), starts_with("Item14")
, starts_with("Item18"), starts_with("Item11")), keys = inv_keys)
cat("Pressure/Tension", ">>", "non-gamified", "\n"); alpha$total

###############################################################
## Wilconxon Analysis                                        ##
###############################################################

rdat <- dplyr::mutate(rdat
, `Interest/Enjoyment` = (rdat$Item12
                          +rdat$Item24
                          +rdat$Item01
                          +rdat$Item09
                          +rdat$Item22
                          +rdat$Item21)/6
, `Perceived Choice` = (48+rdat$Item23
                        +rdat$Item05
                        -(rdat$Item17
                          +rdat$Item15
                          +rdat$Item08
                          +rdat$Item02
                          +rdat$Item06
                          +rdat$Item20))/8
, `Pressure/Tension` = (8+rdat$Item16
                        +rdat$Item14
                        +rdat$Item18
                        -rdat$Item11)/4
)
rdat <- dplyr::mutate(rdat
, `Intrinsic Motivation` = (6*rdat$`Interest/Enjoyment`
                            +8*rdat$`Perceived Choice`
                            +32-4*rdat$`Pressure/Tension`)/18
)

## performing tests in each factor
wt <- wilcox_analysis(rdat$Type, rdat$`Interest/Enjoyment`, alternative = "less", title = "Interest/Enjoyment")
cat("Interest/Enjoyment", "\n"); wt
wt <- wilcox_analysis(rdat$Type, rdat$`Perceived Choice`, alternative = "less", title = "Perceived Choice")
cat("Perceived Choice", "\n"); wt
wt <- wilcox_analysis(rdat$Type, rdat$`Pressure/Tension`, alternative = "greater", title = "Pressure/Tension")
cat("Pressure/Tension", "\n"); wt


wt <- wilcox_analysis(rdat$Type, rdat$`Intrinsic Motivation`, alternative = "less", title = "Intrinsic Motivation")
cat("Intrinsic Motivation", "\n"); wt




