...
dat_prg[['P4']] <- 1*dat_prg[['P4s3']]/4 
dat_prg[['PF']] <- 1*dat_prg[['PFs3']]/4
dat_prg[['PG']] <- 1.25*dat_prg[['PGs3']]/4
dat_prg[['PH']] <- 1.5*dat_prg[['PHs3']]/4
dat_prg <- dat_prg[,c('UserID','P4','PF','PG','PH')]
...
dat[is.na(dat)] <- 0
dat[['score.pre']] <- dat[['score.pre']] + dat[['P4']] 
dat[['score.pos']] <- dat[['score.pos']] + dat[['PF']] + dat[['PG']] + dat[['PH']]
...
title <- "Difference in Scores (Posttest-Pretest) - Recursion"
...
write_nonparam_statistics_analysis_in_latex(
  ...
  , in_title = paste("for Differences of Scores in the third study for signed-up students")
)
...
write_winsorized_in_latex(
  ...
  , in_title = paste("for the differences of scores in the third empirical study for signed-up students")
)
...
write_param_and_nonparam_statistics_analysis_in_latex(
  ...
  , in_title = "in the third study for signed-up students"
