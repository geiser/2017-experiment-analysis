...
title <- "Gains in Skills/Knowledge - Loop Structures"
...
result <- do_nonparametric_test(dat, wid = 'UserID', dv = "gain.theta"
                                , iv = "Type", between = c("Type", "CLRole"))
write_plots_for_nonparametric_test(
  result, ylab = "logits", title = title
...
write_nonparametric_test_report(
  result, ylab = "logits", title = title, override = T
...
if (result$assumptions.fail) {
  ...
  stopifnot('gain.theta' %in% skip_stop)
}
...
write_plots_for_parametric_test(
  result, ylab = "logit", title = title, override = T
