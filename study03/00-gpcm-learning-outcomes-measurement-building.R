...
pre_mdl_strs[[3]] <- c("Re2","Ap1","Ap3","An3b","Ev2")
pre_mdl_strs[[4]] <- c("Un2","Ap1","Ap3","An3b","Ev2")
...
t(pos_info[pos_info$model_fit
           ...
           ,])
pos_mdl_strs <- list()
pos_mdl_strs[[1]] <- c("ReB","UnB","ApA","ApC","AnC2","EvB","PFs3","PGs2")
pos_mdl_strs[[2]] <- c("ReB","ApA","ApC","AnC1","AnC2","EvB","PGs3")
...
if (!file.exists(filename)) {
  write_gpcm_in_latex(
    gpcm_summaries
    , in_title = "for measuring gains in the skills and knowledge of participants in the third empirical study"
    , filename = filename
  )
}
